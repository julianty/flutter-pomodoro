    ...
    final Map<String, Map<String, dynamic>> accumulated = sessionDocs.fold({}, (
      ...
      if (prev[id] == null) {
        ...
        prev[id] = {
          ...
          'label': (cat?.label ?? sessionDoc.categoryLabel)?.replaceAll('none', 'Uncategorized') ?? 'Uncategorized',
              ...
              children: [
                for (var d in data) ...[
                  Builder(builder: (context) {
                    final total = data.fold<int>(0, (sum, d) => sum + (d['duration'] as int));
                    final pct = total > 0 ? ((d['duration'] as int) / total * 100).round() : 0;
                    return Row(
                      children: [
                        Container(
                          width: 12,
